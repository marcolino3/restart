...
import { expenseCategoryNameSchema } from "../schemas/expense-category-form.schema";
...
export function ExpenseCategoriesManager({ categories: initial }: Props) {
  ...
  const onCreate = async () => {
    const parsed = expenseCategoryNameSchema.safeParse(newName);
    if (!parsed.success) {
  ...
  const onUpdateColor = async (id: string, color: string) => {
    ...
    const res = await updateExpenseCategoryAction({ id, color });
    if (!res.success) {
      setCategories((prev) =>
  ...
  const onRename = async (id: string, name: string) => {
    ...
    if (!original || !parsed.success || original.name === parsed.data) return;
    const trimmed = parsed.data;
    const res = await updateExpenseCategoryAction({ id, name: trimmed });
    if (!res.success) {
      setCategories((prev) =>
  ...
  const onDragEnd = async (e: DragEndEvent) => {
    ...
    setCategories(next);
    setBusy(true);
    const res = await reorderExpenseCategoriesAction(next.map((c) => c.id));
    setBusy(false);
    if (!res.success) {
      setCategories(prev);
      toast.error(t("categoryError"), { description: res.error });
